...
import torch.multiprocessing as mp      # 用到了多进程
import time
import numpy as np
...
# Hyperparameters
n_train_processes = 3       # 3个进程
learning_rate = 0.0002      # 学习率
update_interval = 5         # 从每个进程拿5个数据
gamma = 0.98                # 折扣率
max_train_steps = 60000     # 最大训练步数
PRINT_INTERVAL = update_interval * 100  # 每个500步，测试一下环境
...
def worker(worker_id, master_end, worker_end):
    ...
    while True:
        ...
        if cmd == 'step':   # 按照cmd方式控制环境
            ob, reward, done, info = env.step(data)
            if done:
                ob = env.reset()    # 结束后重启环境
            worker_end.send((ob, reward, done, info))
        elif cmd == 'reset':        #环境重置
            ob = env.reset()
            worker_end.send(ob)
        elif cmd == 'reset_task':   # 重置任务
            ob = env.reset_task()
            worker_end.send(ob)
        elif cmd == 'close':        # 关闭进程
            worker_end.close()
...
def test(step_idx, model):
    ...
    print('Step:', step_idx, 'avg score :', score/num_test)
...
if __name__ == '__main__':
    envs = ParallelEnv(n_train_processes)   # 开3个进程同时运行环境

    model = ActorCritic()   # 学习模型
